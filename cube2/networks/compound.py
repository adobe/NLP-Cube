...
class Compound(nn.Module):
    ...
    def __init__(self, config, encodings, num_languages=1, target_device='cpu'):
        ...
        for ii in range(config.encoder_layers):
            ...
            cs_inp = config.encoder_size * 2 + config.lang_emb_size + 16
        ...
        self._start_frame = nn.Embedding(1,
                                         config.encoder_size * 2 + config.char_emb_size + config.lang_emb_size + 16)

    def forward(self, x_chars, x_lang, gs_output=None):
        char_idx, case_idx, lang_idx = self._make_batch(x_chars, x_lang)
        char_emb = self._char_emb(char_idx)
        case_emb = self._case_emb(case_idx)
        lang_emb = self._lang_emb(lang_idx)
        conditioning = torch.cat((case_emb, lang_emb), dim=-1)
    ...
    def _make_batch(self, x_chars, x_langs):
        ...
        return x_char, x_case, x_lang

    def _get_output_idx(self, x):
        tmp = [[0, 0, lemma] for lemma in x]
...
def _extract_data(dataset, unique=True):
    ...
    for example in dataset.sequences:
        ...
        for ii in range(len(seq)):
            if seq[ii].is_compound_entry:
                parts = seq[ii].index.split('-')
                start = int(parts[0])
                stop = int(parts[1])
                l = stop - start + 1
                ...
                if not unique or key not in visited:
                    data.append((w, lang_id, t))
                    visited[key] = 1
            else:
                w = seq[ii].word
                key = (lang_id, w, w)
                if not unique or key not in visited:
                    data.append((w, lang_id, w))
                    visited[key] = 1
...
def _get_targets(batch, encodings):
    lemmas = [example[2] for example in batch]
...
def _start_train(compound, trainset, devset, params):
    import random
    patience_left = params.patience
    trainer = torch.optim.Adam(compound.parameters(), lr=1e-4)
    ...
    while patience_left > 0:
        ...
        for batch in pgb:
            ...
            for ii in range(len(batch)):
                x_chars.append(batch[ii][0])
                x_lang.append(batch[ii][1])
                x_target.append(batch[ii][2])

            y_char_target, y_case_target = _get_targets(batch, encodings=compound._encodings)
            y_char_pred, y_case_pred = compound(x_chars, x_lang, gs_output=x_target)
            y_char_target = torch.tensor(y_char_target, dtype=torch.long, device=compound._get_device()).view(-1)
            y_case_target = torch.tensor(y_case_target, dtype=torch.long, device=compound._get_device()).view(-1)
...
def do_train(params):
    ...
    for ii, train, dev in zip(range(len(train_list)), train_list, dev_list):
        trainset.load_language(train, ii, ignore_compound=False)
        devset.load_language(dev, ii, ignore_compound=False)
    encodings = Encodings()
    if params.resume:
    ...
    config = CompoundConfig(filename=params.config_file)
    config.num_languages = len(train_list)
    compound = Compound(config, encodings, num_languages=len(train_list))
    if params.resume:
        compound.load('{0}.last'.format(params.store))
    compound.to(params.device)
    _start_train(compound, trainset, devset, params)
...
if __name__ == '__main__':
    ...
    parser.add_option('--store', action='store', dest='store', help='Output base', default='comound')
