...
from cube.generic_networks.tokenizers import DummyTokenizer
from cube.io_utils.config import TokenizerConfig

config = TokenizerConfig()

# compute dictionary for known words
dict = {}
f = open('corpus/dict.ja', 'r')
lines = f.readlines()
for word in lines:
    dict[word.replace('\n', '').strip()] = 1
f.close()
# f = open('corpus/cc.ja.300.vec')
# cnt = 0
# for line in f:
#     parts = line.split(' ')
#     dict[parts[0]] = 1
#     cnt += 1
#     if cnt % 10000 == 0:
#         print("Loaded " + str(cnt) + " words")
# f.close()
# f = open('corpus/dict.ja', 'w')
# for word in dict:
#     f.write(word + '\n')
# f.close()
tokenizer = DummyTokenizer(config, encodings, num_languages=len(train_list), dict=dict)
print("Know word count is " + str(len(dict)))
