...
class Lemmatizer(pl.LightningModule):
    ...
    def __init__(self, config, encodings, num_languages=1, target_device='cpu'):
        ...
        self._case_emb = nn.Embedding(4, 16, padding_idx=0)  # 0-pad 1-upper 2-lower 3-symbol
        convolutions = []
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_char_pred = y_char_pred.detach().cpu().numpy()
        for y_pred, y_target in zip(y_char_pred, y_char_target):
            valid = True
            ...
            if valid:
                ok += 1
        total = len(batch)
        return {'loss': loss, 'ok': ok, 'total': total}
