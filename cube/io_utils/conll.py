    ...
    def _make_sequences(self, lines, lang_id=0, ignore_compound=False):
        ...
        for line in lines:
            ...
            if (not line.startswith("#") or in_sequence) and line != '':
                ...
                if len(parts[1]) > 60:
                    seq_ok = False
                    print("Skipping sentence due to word length: {}".format(parts[1]))
                ...
                if seq_ok is False:
                    skipped_sequences += 1
                if len(seq) > 0 and seq_ok is True:
                    sequences.append([seq, lang_id])
                seq = []
                seq_ok = True
        if skipped_sequences>0:
            print("\n\t ... skipped {} invalid sentences in this dataset.".format(skipped_sequences))
        return sequences
