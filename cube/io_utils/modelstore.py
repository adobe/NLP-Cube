...
from cube.io_utils.components import Component, TokenizerComponent, MWExpanderComponent, POSTaggerComponent, \
    LemmatizerComponent, ParserComponent, NERComponent
...
class ModelStore():
    ...
    def solve(lang: str, check_for_updates: bool = False) -> dict:
        ...
        catalog = ModelStore._get_catalog(
            url="https://raw.githubusercontent.com/adobe/NLP-Cube-Models/2.0/catalog.json")

        # get model filename
        model_name = None
        for language in catalog['languages']:
            if language[1] == lang:
                model_url = '{0}/{1}'.format(catalog['base_url'], language[2])
                model_name = language[2]
                model_lang_id = language[3]
                print(model_name, model_lang_id)
                break

        if not model_name:
            raise Exception(
                "Model for language '{}' not found in catalog. Please check on the official site for support for this language.".format(
                    lang))
        ...
        if os.path.exists(
            ...
            paths["tokenizer"] = {
                ...
                "encodings": os.path.join(ModelStore.root_path, "models", model_name,
                                          "tokenizer-{}.encodings".format(model_name)),
            }

        # mwe paths
        if os.path.exists(
                os.path.join(ModelStore.root_path, "models", model_name, "compound-{}.best".format(model_name))):
            paths["compound"] = {
                "model": os.path.join(ModelStore.root_path, "models", model_name,
                ...
                "encodings": os.path.join(ModelStore.root_path, "models", model_name,
                                          "compound-{}.encodings".format(model_name)),
            }

        # lemmatizer paths
        if os.path.exists(
                os.path.join(ModelStore.root_path, "models", model_name, "lemmatizer-{}.best".format(model_name))):
            paths["lemmatizer"] = {
                "model": os.path.join(ModelStore.root_path, "models", model_name,
                                      "lemmatizer-{}.best".format(model_name)),
                "config": os.path.join(ModelStore.root_path, "models", model_name,
                                       "lemmatizer-{}.conf".format(model_name)),
                "encodings": os.path.join(ModelStore.root_path, "models", model_name,
                                          "lemmatizer-{}.encodings".format(model_name)),
            }

        # tagger paths
        if os.path.exists(
                os.path.join(ModelStore.root_path, "models", model_name, "tagger-{}.bestUPOS".format(model_name))):
            paths["tagger"] = {
                "model_UPOS": os.path.join(ModelStore.root_path, "models", model_name,
                                           "tagger-{}.bestUPOS".format(model_name)),
                "model_XPOS": os.path.join(ModelStore.root_path, "models", model_name,
                                           "tagger-{}.bestXPOS".format(model_name)),
                "model_ATTRS": os.path.join(ModelStore.root_path, "models", model_name,
                                            "tagger-{}.bestATTRS".format(model_name)),
                "config": os.path.join(ModelStore.root_path, "models", model_name, "tagger-{}.conf".format(model_name)),
                "encodings": os.path.join(ModelStore.root_path, "models", model_name,
                                          "tagger-{}.encodings".format(model_name)),
            }

        # parser paths
        if os.path.exists(
                os.path.join(ModelStore.root_path, "models", model_name, "parser-{}.bestUAS".format(model_name))):
            paths["parser"] = {
                "model": os.path.join(ModelStore.root_path, "models", model_name,
                                      "parser-{}.bestUAS".format(model_name)),
                "config": os.path.join(ModelStore.root_path, "models", model_name, "parser-{}.conf".format(model_name)),
                "encodings": os.path.join(ModelStore.root_path, "models", model_name,
                                          "parser-{}.encodings".format(model_name)),
            }

        return paths, model_lang_id
...
try:
    ...
    logger.error("Could not find model store / could not create path in '{}', failed with exception: {}".format(
        ModelStore.root_path, str(ex)))
